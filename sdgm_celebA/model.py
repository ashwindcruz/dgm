...
class VAE(chainer.Chain):
    def __init__(self, dim_in, dim_hidden, dim_latent, num_layers, temperature, num_zsamples=1):
        ...
        # batch normalization layers at the start of the encoder and start of the decoder, auxiliary variable
        self.qlina_batch_norm_0 = L.BatchNormalization(dim_hidden)
        self.plina_batch_norm_0 = L.BatchNormalization(dim_hidden)
        self._children.append('qlina_batch_norm_0')
        self._children.append('plina_batch_norm_0')

        # batch normalization layers at the start of the encoder and start of the decoder, z and x variable
        self.qlinz_batch_norm_0 = L.BatchNormalization(dim_hidden)
        self.plinx_batch_norm_0 = L.BatchNormalization(dim_hidden)
        self._children.append('qlinz_batch_norm_0')
        self._children.append('plinx_batch_norm_0')
        ...
        for i in range(num_layers-1):
            ...
            layer_name = 'qlina_batch_norm_' + str(i+1)
    ...
    def encode_a(self, x):
        a_params = self.qlina0(x)
        a_params = self.qlina_batch_norm_0(a_params)
        a_params = F.crelu(a_params)

        for i in range(self.num_layers-1):
            layer_name = 'qlina' + str(i+1)
            a_params = self[layer_name](a_params)
            layer_name = 'qlina_batch_norm_' + str(i+1)
            a_params = self[layer_name](a_params)
            a_params = F.crelu(a_params)
